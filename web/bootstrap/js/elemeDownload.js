(function($){
	...
	//crawlerEvaluate顾客评价下载
	$(".eleme .evaluateDownload").on("click",function(){
			//发送请求
			$.ajax({
				url:address+"evaluateDownload",//请求路径
				type:"get",//请求方式 get和post
				dataType:"json",//返回数据的格式
				success:function(e){
					//e 为请求成功后返回的数据
				...
				error:function(){
					//请求失败时的函数
					alert("下载失败");
					$(this).parents("tr").find(".getTime").text("请求失败");
					console.log("请求失败");//控制台输出请求失败
				}
			})
		})
